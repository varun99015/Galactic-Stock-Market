...
const Settings = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-cyan-100 p-3 md:p-6 relative overflow-hidden">
      {/* Sci-fi background elements */}
      <div className="fixed inset-0 z-0 opacity-20">
        <div className="absolute inset-0 bg-[url('https://assets.codepen.io/13471/holo-grid.png')] bg-[size:30px_30px] md:bg-[size:50px_50px]"></div>
        {[...Array(15)].map((_, i) => (
          <div 
            key={i}
            className="absolute bg-cyan-500 rounded-full"
      ...
      <div className="relative z-10 max-w-6xl mx-auto bg-black/70 backdrop-blur-lg border border-cyan-500/30 rounded-xl overflow-hidden shadow-2xl">
        {/* Header */}
        <div className="p-4 md:p-6 border-b border-cyan-500/20 bg-gradient-to-r from-cyan-900/30 to-blue-900/30">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center space-y-3 md:space-y-0">
            <h1 className="text-xl md:text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-300 to-blue-400 font-mono tracking-wider">
              SYSTEM CONFIG
            </h1>
            <div className="flex items-center space-x-2 md:space-x-3">
              <span className="px-2 py-1 md:px-3 md:py-1 bg-cyan-900/50 text-cyan-300 rounded-full text-xs font-mono flex items-center">
                <span className="w-1.5 h-1.5 md:w-2 md:h-2 bg-green-400 rounded-full mr-1 md:mr-2 animate-pulse"></span>
                {accessGranted ? 'GRANTED' : 'PENDING'}
              </span>
              <button 
                onClick={verifyIdentity}
                disabled={accessGranted}
                className={`px-2 py-1 md:px-3 md:py-1 rounded-full text-xs font-mono border ${
                  accessGranted 
                    ? 'border-green-500 text-green-300' 
                    : 'border-yellow-500 text-yellow-300 hover:bg-yellow-900/20'
                } transition-colors`}
              >
                {accessGranted ? 'VERIFIED' : 'VERIFY'}
              </button>
            </div>
          </div>
          <p className="mt-1 md:mt-2 text-cyan-300/80 font-mono text-xs md:text-sm">
            {accessGranted 
              ? 'Full system configuration access authorized' 
        ...
        {/* Mobile menu button */}
        <div className="md:hidden p-3 border-b border-cyan-500/20 bg-cyan-900/20">
          <button
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
            className="w-full py-2 px-3 bg-cyan-800/50 text-cyan-300 rounded-md font-mono text-sm flex items-center justify-between"
          >
            <span>{activeTab.toUpperCase()} SETTINGS</span>
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} md:block w-full md:w-56 border-b md:border-b-0 md:border-r border-cyan-500/20 bg-black/40`}>
            ...
            {/* Voice command input */}
            <div className="p-3 md:p-4 border-t border-cyan-500/20 mt-2 md:mt-4">
              <div className="font-mono text-xs text-cyan-300 mb-2">VOICE COMMAND</div>
              <div className="flex">
                <input
                  type="text"
                  value={voiceCommand}
                ...
                <button
                  onClick={processVoiceCommand}
                  className="bg-cyan-600/50 hover:bg-cyan-500/70 px-2 md:px-3 rounded-r-md text-xs font-mono border-t border-b border-r border-cyan-500/30"
                >
                  EXEC
                </button>
              </div>
          ...
          <div className="flex-1 p-4 md:p-6">
            {!accessGranted && activeTab !== 'user' ? (
              <div className="flex flex-col items-center justify-center h-48 md:h-64 text-center">
                <div className="w-16 h-16 md:w-20 md:h-20 border-4 border-yellow-500 rounded-full animate-pulse mb-3 md:mb-4 flex items-center justify-center">
                  <div className="w-12 h-12 md:w-16 md:h-16 border-4 border-yellow-500/50 rounded-full animate-pulse"></div>
                </div>
                <h3 className="text-lg md:text-xl text-yellow-300 font-mono mb-1 md:mb-2">SECURITY LOCK</h3>
                <p className="text-yellow-200/80 text-xs md:text-sm max-w-md px-2">
                  Biometric verification required to access {activeTab} settings.
                </p>
              </div>
            ) : (
              <>
                ...
                {activeTab === 'user' && (
                  <div className="space-y-4 md:space-y-6">
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
                      <SettingCard 
                        title="USER PROFILE" 
                        description="Personal identification parameters"
                      >
                        <div className="space-y-3 md:space-y-4">
                          <div>
                            ...
                            <select
                              value={settings.theme}
                              onChange={(e) => setSettings({...settings, theme: e.target.value})}
                              className="w-full bg-black/50 border border-cyan-500/30 text-cyan-100 p-2 rounded-md focus:outline-none focus:ring-1 focus:ring-cyan-500 text-xs md:text-base"
                            >
                              <option value="nebula">Nebula Hologram</option>
                              <option value="quantum">Quantum Particle</option>
                              <option value="cyber">Cyber Interface</option>
                      ...
                      <SettingCard 
                        title="PREFERENCES" 
                        description="Interaction parameters"
                      >
                        <div className="space-y-3 md:space-y-4">
                          <ToggleSetting
                            label="HOLOGRAPHIC UI"
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6">
                      <SettingCard 
                        title="BIOMETRICS" 
                        description="Biological identification"
                      >
                        <div className="space-y-3 md:space-y-4">
                          <ToggleSetting
                            label="RETINA SCAN LOGIN"
                      ...
                      <SettingCard 
                        title="DATA CONFIG" 
                        description="Information flow parameters"
                      >
                        <div className="space-y-3 md:space-y-4">
                          <div>
                            <label className="block text-cyan-300 text-xs font-mono mb-1">DATA STREAM</label>
                ...
                {activeTab === 'interface' && (
                  <div className="space-y-4 md:space-y-6">
                    <SettingCard 
                      title="VISUAL PROTOCOLS" 
                      description="Display rendering parameters"
                    >
                          ...
                          <div className="grid grid-cols-2 gap-1 md:grid-cols-3 md:gap-2">
                            {['nebula', 'cyber', 'quantum', 'dark-matter', 'void', 'neon'].map((theme) => (
                              <div 
                                key={theme}
                                onClick={() => setSettings({...settings, theme})}
                                className={`h-10 md:h-12 rounded-md cursor-pointer border-2 ${
                                  settings.theme === theme
                                    ? 'border-cyan-400 shadow-lg shadow-cyan-500/20'
                                    : 'border-gray-700 hover:border-gray-600'
                                } bg-gradient-to-br ${
                    ...
                    <SettingCard 
                      title="HUD CONFIG" 
                      description="Heads-up display parameters"
                    >
                ...
                {/* Action buttons */}
                <div className="mt-6 md:mt-8 flex flex-col-reverse md:flex-row justify-end space-y-reverse space-y-2 md:space-y-0 md:space-x-3">
                  <button
                    onClick={() => navigate(-1)}
                    className="px-4 py-2 border border-gray-600 text-gray-300 rounded-md hover:bg-gray-800/50 transition-colors font-mono text-sm md:text-base"
                  >
                    CANCEL
                  </button>
                  <button
                    onClick={saveSettings}
                    disabled={saving}
                    className="px-4 py-2 bg-cyan-600/70 hover:bg-cyan-500/80 rounded-md text-white font-mono flex items-center justify-center disabled:opacity-50 transition-all text-sm md:text-base"
                  >
                    {saving ? (
                      <>
                        <svg className="animate-spin -ml-1 mr-2 h-4 w-4 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
const ToggleSetting = ({ label, checked, onChange, disabled = false }) => (
  <div className="flex items-center justify-between py-1">
    <span className="text-cyan-200 text-xs md:text-sm">{label}</span>
    <button
      onClick={onChange}
      disabled={disabled}
    ...
    >
      <span
        className={`inline-block h-3 w-3 md:h-4 md:w-4 transform rounded-full bg-white transition-transform ${
          checked ? 'translate-x-4 md:translate-x-6' : 'translate-x-1'
        }`}
      />
    </button>
  </div>
