...
const Profile = () => {
  ...
  return (
    <div className="font-mono relative w-full min-h-screen flex flex-col p-4 md:p-5 justify-start space-y-5 text-[#2ca5eb] bg-black/50 backdrop-blur-md overflow-hidden">
      
      {/* Border and circuit board background */}
      <div className="absolute inset-0 border-2 border-[#237da7] opacity-70 overflow-hidden rounded-lg md:rounded-none">
        <div className="absolute inset-0 bg-[url('https://assets.codepen.io/13471/circuit-board.svg')] opacity-10"></div>
        <div className="absolute top-0 left-0 w-2 h-2 bg-[#237da7] animate-pulse"></div>
        <div className="absolute top-0 right-0 w-2 h-2 bg-[#237da7] animate-pulse"></div>
        <div className="absolute bottom-0 left-0 w-2 h-2 bg-[#237da7] animate-pulse"></div>
      ...
      {/* Main Deck Navigation - Mobile Responsive */}
<div className="absolute top-3 left-3 z-20 flex items-center 
                bg-cyan-900/30 hover:bg-cyan-800/50 
                rounded-lg border border-cyan-500/30 hover:border-cyan-400 
                transition-all group mt-5
                w-10 h-10 sm:w-8 sm:h-8 md:w-5 md:h-2">
  <MainDeck/>
</div>

      {/* Holographic user profile section */}
      <div className='flex flex-col justify-center items-center relative z-10 mt-10 md:mt-0'>
        {/* Interactive holographic avatar with toggle */}
        <div className="relative mb-3 group cursor-pointer" onClick={toggleHologram}>
          <div className={`absolute inset-0 rounded-full ${hologramActive ? 'bg-[#00FFFF]/20' : ''} 
            ${hologramActive ? 'animate-[hologram_3s_ease-in-out_infinite]' : ''}`}></div>
          <img 
            src={bg} 
            alt="user" 
            className={`rounded-full h-20 w-20 md:h-24 md:w-24 border-2 ${hologramActive ? 'border-[#00FFFF]' : 'border-[#237da7]'} 
              object-cover transition-all duration-300 ${hologramActive ? 'grayscale-[80%]' : ''}`}
          />
          <div className={`absolute -inset-2 rounded-full border-2 ${hologramActive ? 'border-[#00FFFF]' : 'border-[#237da7]'} 
            opacity-50 ${hologramActive ? 'animate-[pulse_2s_infinite]' : 'animate-[pulse_3s_infinite]'}`}></div>
        ...
        {/* Animated greeting with typewriter effect */}
        <h3 className="text-base md:text-lg font-mono mb-2 relative text-center px-2">
          <span className={`inline-block overflow-hidden whitespace-nowrap border-r-2 border-r-[#00FFFF] 
            ${scanComplete ? 'w-auto border-r-0' : 'w-0'} 
            animate-[typing_2s_steps(20,end)_forwards]`}>
            {user ? `IDENTIFICATION CONFIRMED` : `SCANNING BIOSIGNATURES...`}
        ...
        {/* Interactive user data terminal */}
        <div className={`font-mono text-xs md:text-sm bg-black/30 p-3 rounded-lg border border-[#237da7]/30 
          w-full max-w-xs transition-all duration-500 ${scanComplete ? 'h-auto' : 'h-16 md:h-20'}`}>
          {user ? (
            <div className="space-y-1 md:space-y-2">
              <p className="text-[#00FFFF] flex items-center">
                <span className="inline-block w-3 h-3 md:w-4 md:h-4 bg-[#00FFFF] mr-1 md:mr-2 animate-pulse"></span>
                USER: <span className="text-white ml-1 md:ml-2 truncate">{user.name}</span>
              </p>
              <p className="text-[#00FFFF] flex items-center">
                <span className="inline-block w-3 h-3 md:w-4 md:h-4 bg-[#00FFFF] mr-1 md:mr-2 animate-pulse delay-100"></span>
                ACCESS: <span className={`ml-1 md:ml-2 ${
                  accessLevel === 'OMEGA' ? 'text-purple-400' : 
                  accessLevel === 'ALPHA' ? 'text-red-400' : 
                  accessLevel === 'BETA' ? 'text-yellow-400' : 
                  'text-green-400'
                } font-bold`}>{accessLevel}</span>
              </p>
              <p className="text-[#00FFFF] flex items-center">
                <span className="inline-block w-3 h-3 md:w-4 md:h-4 bg-[#00FFFF] mr-1 md:mr-2 animate-pulse delay-200"></span>
                LOGIN: <span className="text-white ml-1 md:ml-2 text-xs">{(new Date()).toLocaleString()}</span>
              </p>
              <div className="mt-2 md:mt-3 pt-2 border-t border-[#237da7]/20">
                <div className="flex justify-between text-xs">
            ...
            <div className="h-full flex items-center justify-center">
              <div className="flex space-x-2">
                <div className="w-2 h-2 md:w-3 md:h-3 bg-[#00FFFF] rounded-full animate-bounce"></div>
                <div className="w-2 h-2 md:w-3 md:h-3 bg-[#00FFFF] rounded-full animate-bounce delay-100"></div>
                <div className="w-2 h-2 md:w-3 md:h-3 bg-[#00FFFF] rounded-full animate-bounce delay-200"></div>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Security verification indicator */}
        {scanComplete && (
          <div className="mt-2 md:mt-3 text-xs px-3 py-1 bg-green-900/30 text-green-300 rounded-full flex items-center">
            <span className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse"></span>
            BIOMETRIC VERIFIED
          </div>
        )}
      ...
      {/* Navigation with advanced sci-fi effects */}
      <div className="space-y-2 md:space-y-3 mt-4 md:mt-5 relative z-10">
        <Link 
          to="/dashboard" 
          className="block py-2 px-3 border-b border-[#237da7]/30 hover:bg-[#237da7]/10 hover:border-[#193c53] transition-all duration-300 group relative overflow-hidden"
        >
          <span className="absolute left-0 top-0 h-full w-1 bg-[#00FFFF] opacity-0 group-hover:opacity-100 transition-opacity"></span>
          <span className="text-[#00FFFF] font-mono text-sm md:text-base mr-2 group-hover:text-white flex items-center">
            <span className="w-2 h-2 bg-[#00FFFF] mr-2 group-hover:animate-pulse"></span>
            COMMAND DASHBOARD
          </span>
        </Link>
      ...
      {/* Interactive grid overlay */}
      <div className="absolute inset-0 pointer-events-none opacity-5">
        <div className="absolute inset-0 bg-[url('https://assets.codepen.io/13471/holo-grid.png')] bg-[length:30px_30px] md:bg-[length:40px_40px]"></div>
      </div>
    </div>
  );
}
