...
import {SpaceHelmet,BlackHole,Alien,Radiation} from "../constants/Custom Icons";// Import your dataset
    ...
    <>
      {stock.technicalSpecs && (
        <><h1 className=' mb-4 text-cyan-500'>Technical Specs</h1>
          <p>ColoniesBuilt: {data.coloniesBuilt}</p>
      ...
      {stock.sciFiLore && (
        <><h1 className='mt-4 pt-2 mb-4 text-cyan-500 border-t border-cyan-700'>Scifi Lore</h1>
      ...
      {colonizationSectors.map((sector, index) => (
        ...
        >
          ...
          <p className="text-cyan-00">{sector.type[0]}</p>
          <p className="text-cyan-200">{sector.type[1]}</p>
        </div>
      ))}
    </div>
          </div>
