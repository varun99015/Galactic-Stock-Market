...
const FXSTOCK5 = ({ stock }) => {
    ...
    const fetchStockData = async () => {
      try {
        ...
        if (response.data && response.data.length > 0) {
          const formattedData = response.data.map(d => ({...d, date: new Date(d.date)}));
          setHistoricalData(formattedData);
          setStockData(formattedData[formattedData.length - 1]);
        }
      } catch (err) {
  ...
  return (
    <div className='min-h-fit'>
      {/* Responsive Change: Adjusted padding for mobile */}
      <div className="min-h-screen bg-gradient-to-br from-black via-gray-900 to-black text-cyan-300 font-mono p-4 md:p-8">
        ...
        <div className="relative z-10 text-center mb-10">
          ...
          <h1 className="text-3xl md:text-4xl font-bold tracking-widest">{stock.name}</h1>
          <p className="text-sm text-cyan-400">Intergalactic Stock Exchange</p>
        </div>

        {/* Section 1: Stock Info in a Two-Column Grid */}
        <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 mb-10">
          {/* Left Panel: Metrics and Details */}
          <div className="space-y-4 border border-cyan-500 p-4 md:p-6 rounded-2xl shadow-lg bg-opacity-10 bg-cyan-900 backdrop-blur-sm">
            ...
            <div className="mt-4 pt-4 border-t border-cyan-800">
              ...
              <div className="mr-1">
                {stock.competitors?.map((name, idx) => (
                  <p 
                    key={idx} 
                    onClick={() => navigate(`/f_stock/spaceStocks/${name}`)} 
                    className="cursor-pointer hover:text-cyan-200 py-1 text-sm md:text-base"
                  >
                    {name}
                  </p>
                ))}
              ...
              <div className='text-cyan-300 mb-3 mt-4 pt-4 border-t border-cyan-800 text-xs md:text-sm'>
                {(() => {
                  const data = stock.technicalSpecs;
                  if (!data) return null;
                  const s = stock.sciFiLore;

                  return (
                    <>
                      {stock.technicalSpecs && (
                        <><h1 className='mb-4 text-cyan-500'>Technical Specs</h1>
                          <p>Colonies Built: {data.coloniesBuilt}</p>
                      ...
                      {stock.sciFiLore && (
                        <><h1 className='mt-4 pt-2 mb-4 text-cyan-500 border-t border-cyan-700'>Sci-Fi Lore</h1>
                          <p>Achievement: {s.achievement}</p>
                          <p>Scandal: {s.scandal}</p>
                        </>
                      )}
                    </>
                  );
              ...
              <h3 className="mt-4 pt-4 border-t border-cyan-800 text-cyan-400 mb-2 text-sm md:text-base">COLONIZATION TAXONOMY</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 md:p-6">
                {colonizationSectors.map((sector, index) => (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <p className="text-xs md:text-sm text-cyan-200">{sector.type[0]}</p>
                    <p className="text-xs md:text-sm text-cyan-200">{sector.type[1]}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        {/* Section 2: Full-Width Chart and Trading Form */}
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-1 gap-6 md:gap-10 mb-10">
          {/* Chart takes up full width */}
          <div className="lg:col-span-1 border border-cyan-500 p-4 md:p-6 rounded-2xl shadow-lg bg-opacity-10 bg-cyan-900 backdrop-blur-sm">
            ...
            <div className="flex flex-col sm:flex-row sm:justify-between mt-4 text-xs text-cyan-400 gap-2 sm:gap-0">
              <div className="flex items-center">
